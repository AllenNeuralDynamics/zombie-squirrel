...
from biodata_cache.cache_table_helpers.platform_ecephys_units import _find_nwb_prefixes, _parse_s3
...
def _load_zmetadata(client, bucket: str, nwb_prefix: str) -> tuple[bytes, dict] | None:
    """Return the raw consolidated metadata bytes and parsed dict for one NWB store."""
    body = client.get_object(Bucket=bucket, Key=f"{nwb_prefix}/.zmetadata")["Body"].read()
    if len(body) == 0:
        return None
    try:
        return body, json.loads(body).get("metadata", {})
    except json.JSONDecodeError:
        return None


def _download_arrays(client, bucket: str, nwb_prefix: str, zmetadata: bytes, array_paths: list[str]) -> dict:
    """Download every chunk under each of ``array_paths`` (NWB-relative, no leading/trailing slash).

    Returns an in-memory zarr store dict keyed by NWB-relative path, ready for
...
def _read_group_columns(client, bucket: str, nwb_prefix: str, zmetadata: bytes, metadata: dict, group: str, columns: list[str]) -> dict | None:
    ...
    store = _download_arrays(client, bucket, nwb_prefix, zmetadata, paths)
...
def _discover_asset(location: str) -> tuple[str, str, dict, bytes, dict, list[dict]] | None:
    ...
    for nwb_prefix in _find_nwb_prefixes(client, bucket, key):
        loaded = _load_zmetadata(client, bucket, nwb_prefix)
        if loaded is None:
            continue
...
def _asset_rows(
    ...
    spike_arrays = _download_arrays(client, bucket, nwb_prefix, zmetadata, ["units/spike_times", "units/spike_times_index"])
    import zarr
